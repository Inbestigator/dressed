...
export async function sendSound(
  ...
  const res = await callDiscord(
    Routes.sendSoundboardSound(channel),
    {
      method: "POST",
      body: data,
    },
 ...
 */
export async function listSounds(guild?: Snowflake): Promise<
  RESTGetAPISoundboardDefaultSoundsResult
> {
  const res = await callDiscord(
    guild
 ...
 */
export async function getSound(guild: Snowflake, sound: Snowflake): Promise<
  RESTGetAPIGuildSoundboardSoundResult
> {
  const res = await callDiscord(
    Routes.guildSoundboardSound(guild, sound),
    {
      method: "GET",
    },
  );
