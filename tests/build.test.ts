...
const withInstance =
  `import { createServer, setupCommands, setupComponents } from "@dressed/dressed/server";

export const commandData = [{"name":"ping","import": ()=>import("./tests/src/commands/ping.ts")}];
export const componentData = [{"name":"button_[arg]","category":"buttons","regex":"^button_(?<arg>.+)$","import": ()=>import("./tests/src/components/buttons/button_[arg].ts")}];
export const config = {"root":"tests/src"};

createServer(setupCommands(commandData), setupComponents(componentData), config);`;
...
const withBoth =
  `import { createServer, installCommands, setupCommands, setupComponents } from "@dressed/dressed/server";
